...
The platform uses **Cloudinary** for optimized image handling:

### Features
- **Automatic optimization**: Images resized to max 1200px width, compressed to ~80% quality
- **Thumbnail generation**: 400px width thumbnails created for fast loading in listings
- **Format conversion**: Automatic WebP/JPEG conversion for best browser support
- **Lazy loading**: All carousel images lazy-load except the first visible image

### API Endpoints
- `POST /api/cloudinary/upload` - Upload single image (returns thumbnailUrl + fullUrl)
- `POST /api/cloudinary/upload-gallery` - Upload multiple gallery images
- `PUT /api/companies/:id/cloudinary-logo` - Save logo URLs to company
- `PUT /api/companies/:id/cloudinary-gallery` - Save gallery URLs to company

### Helper Module
`server/cloudinaryService.ts` provides:
- `uploadOptimizedImage(buffer, options)` - Upload from buffer
- `uploadOptimizedImageFromUrl(url, options)` - Upload from URL
- Returns: `{ thumbnailUrl, fullUrl, publicId }`
...
### Migration Script
Run `npx tsx scripts/migrate-images-to-cloudinary.ts` to migrate existing images to Cloudinary.

### Database Fields
- `logo_url` - Full resolution logo URL
- `logo_thumbnail_url` - Thumbnail logo URL (400px)
- `gallery_images` - Array of full resolution gallery URLs
- `gallery_thumbnails` - Array of thumbnail gallery URLs

## External Dependencies
- **PostgreSQL**: Primary database (Replit internal, DATABASE_URL secret).
